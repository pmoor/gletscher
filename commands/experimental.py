...
def experimental_command(args):
  ...
  for job in jobs:
    if job["Action"] == "InventoryRetrieval":
      creation_date = datetime.datetime.strptime(job["CreationDate"], "%Y-%m-%dT%H:%M:%S.%fZ")
      completion_date = None
      age = None
      if job["CompletionDate"]:
        completion_date = datetime.datetime.strptime(job["CompletionDate"], "%Y-%m-%dT%H:%M:%S.%fZ")
      ...
      if job["Completed"] and job["StatusCode"] == "Succeeded":
        job_id = job["JobId"]
        ...
        for archive in list["ArchiveList"]:
          ...
          if id == config.uuid():
            if description["type"] == "data":
              ...
              tree_hash = description["tree-hash"].decode("hex")
